...
Defines how much fees were collected from already performed swap.

### SwapReferralFee

```tl-b
swap_referral_fee referral:MsgAddressInt input_amount:Coins
                  output_amount:Coins = SwapReferralFee;
```

Defines how much fees were collected from already performed swap to the specified referral.

### SwapFees

```tl-b
swap_fees lp:SwapGenericFee protocol:SwapGenericFee
          referral:(Maybe ^SwapReferralFee) = SwapFees;
```

Defines how much fees were collected from already performed swap in total.

| Property Name | Meaning                                                                                                                   |
|---------------|---------------------------------------------------------------------------------------------------------------------------|
| lp            | Fees for liquidity providers (they remain in pool reserves indefinitely)                                                  |
| protocol      | Fees for the protocol (they remain in pool reserves temporary, but will be withdrawn at some point in the future)         |
| referral      | Fees for the referral, if any (they remain in pool reserves temporary, but will be withdrawn at some point in the future) |
...
swap_successful_event#c0ffee30 query_id:uint64 input:Asset input_amount:Coins
                               output_amount:Coins reserves:^PoolReserves
                               fees:^SwapFees = SwapSuccessfulEvent;
```
...
| fees          | Information about fees that were collected with this swap operation                                                                   |
...
Is being sent by the **Native Vault** and **Extra-Currency Vaults** in **some** cases. Specifically, it occurs if:
...
Used to withdraw protocol and referral fees: they are not distributed (nor calculated) on-chain,
because it will lead to significantly higher gas consumption for end-users.
