...
Each pool is uniquely identified by its assets pair AND AMM strategy. It means that within a single implementation of
**swap.coffee DEX** there could not exist two pools of the same AMM strategy and, for example, an assets pair of TON/USDT.
...
Either known as "constant product" or "Uniswap" AMM. Ensures that product of pool's reserves remains unchanged after
...
between this AMM strategy and volatile ones.
...
For our purely TVM-based implementation of this strategy, we use the following constants for the formulas introduced in the documentation above:

- `n = 2`
- `A = 2000`

Thus, formula takes the following form: `8000 * (x + y) = 7999 * D + D^3 / (4xy)`, where `x` and `y` are pool's reserves
and `D` is some predetermined variable.
...
As described below, they're being uniquely identified by the asset pair and AMM strategy type.
